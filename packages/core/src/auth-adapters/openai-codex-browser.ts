...
export type CodexCliRunner = (args: string[]) => Promise<CodexCliCommandResult>;

export interface OpenAICodexBrowserAdapterOptions {
...
function toSafeMessage(reason: "unavailable" | "failed" | "unsupported", action: string): string {
  ...
  if (reason === "unsupported") {
    return `Codex browser auth ${action} is not supported in this environment`;
  }
  return `Codex browser auth ${action} failed`;
}
...
function parseStatusOutput(stdout: string, stderr: string): AuthStatusResult {
  try {
    ...
    if (parsed["authenticated"] === true || parsed["loggedIn"] === true) {
      return { status: "authenticated", message: "Codex CLI is authenticated" };
    }
...
function supportsOpenAIBrowser(context: AuthAdapterContext): boolean {
  ...
  return (
    providerKind === "openai" ||
    providerKey === "openai" ||
    context.profile.accountType === "chatgpt-plus" ||
    context.profile.accountType === "chatgpt-pro"
  );
}

export function formatCodexAuthStatusForCli(status: AuthStatusResult): string {
  return `${status.status}: ${status.message}`;
}

export function createOpenAICodexBrowserAuthAdapter(
  options: OpenAICodexBrowserAdapterOptions = {},
): AuthProviderAdapter {
  const runCodexCli = options.runCodexCli ?? defaultCodexRunner;
  ...
  async function getStatus(): Promise<AuthStatusResult> {
    ...
    return parseStatusOutput(result.stdout, result.stderr);
  }
  ...
  async function logout(): Promise<AuthStatusResult> {
    ...
    return { status: "not_authenticated", message: "Codex CLI is logged out" };
  }

  return {
    name: "openai-codex-browser",
    supports: supportsOpenAIBrowser,
    async checkHealth(context): Promise<AuthHealthCheckResult> {
      ...
      if (status.status === "authenticated") {
        return {
          ...
          checks: [...checks, { key: "codex-auth", status: "pass", detail: status.message }],
        };
      }
      if (status.status === "not_authenticated") {
        return {
          state: "degraded",
          authStatus: status.status,
          message: `Auth profile ${context.profileKey} requires Codex login`,
          checks: [...checks, { key: "codex-auth", status: "warn", detail: status.message }],
        };
      }
      return {
        state: "degraded",
